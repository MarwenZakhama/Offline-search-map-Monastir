...
- **Search never hides anything.** Matching points turn green and keep their reference; every other point stays on the map, faded — and its reference stays too, faded the same way, so nothing disappears while you type. In a crowded spot, a match's reference is shown even if it has to sit on top of a neighbouring point or label; a non-matching reference still steps aside if there's no room.
...
- **All map icons and names are faded on purpose**, street and town names included, so they never look like your own points. To make them stronger or fainter, change `MAP_FADE` at the top of `mapstyle.js` (0.42 now; 1 = full strength).
- **Main roads are light yellow and secondary roads an even paler, faded yellow** — enough to tell them apart from local streets (which stay white) without standing out. The railway stays faint too. Change the colours in `osmRoadColors` inside `mapstyle.js` if you want a different shade.
