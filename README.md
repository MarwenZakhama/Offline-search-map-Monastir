...
- **Search never hides anything.** Matching points turn green and keep their reference; every other point stays on the map, faded — and its reference stays too, faded the same way, so nothing disappears while you type.
...
- **Roads, the railway and administrative divisions are coloured like openstreetmap.org**: main roads orange, secondary roads yellow, small streets white, all with a darker outline; the railway is a dark dashed line; town/neighbourhood boundaries are dashed magenta. Colours and widths are set in `osmRoadColors` inside `mapstyle.js`.
